(function(global) {
    ...
    function dstRead(file, pattern) {
        ...
        var string = enc.decode(file.buffer);
        var dataString = string.match(/#[a-fA-F0-9]{6}|#[a-fA-F0-9]{3}/g);
        console.log(dataString);
        pattern.colors = [];
        for (var cc = 0; cc < dataString.length; cc++) {
            pattern.colors.push(hexToRgbA(dataString[cc]));
            console.log(pattern.colors);
        }
        console.log(pattern.colors);
        //pattern.colors = dataString;
        file.seek(512);
